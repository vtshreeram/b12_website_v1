      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="bg-white rounded-3xl p-8 md:p-12 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 flex flex-col md:flex-row items-center justify-between gap-10">
          <div className="md:w-1/3">
            <h2 className="text-[clamp(1.25rem,2.5vw,1.75rem)] font-medium tracking-tight text-gray-900 leading-[1.15] text-balance">Empowering leading<br />healthcare providers.</h2>
          </div>
          <div className="md:w-2/3 grid grid-cols-2 md:grid-cols-4 gap-8">
            {metrics.map((metric) => (
              <div key={metric.label}>
                <div className="text-[clamp(1.5rem,3vw,2rem)] font-semibold text-gray-900 mb-1 leading-none">
                  {metric.value}
                  {metric.unit && <span className="text-blue-600">{metric.unit}</span>}
                </div>
                <div className="text-[clamp(0.8rem,1.25vw,0.95rem)] text-gray-500 font-medium">{metric.label}</div>
              </div>
            ))}
          </div>
        </div>
